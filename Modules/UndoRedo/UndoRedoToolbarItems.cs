...
namespace Gemini.Modules.UndoRedo
{
    ...
    {
        public static async Task ExecuteUndo()
        {
            await new LambdaResult(c => IoC.Get<IShell>().ActiveItem.UndoRedoManager.Undo(1)).ExecuteAsync();
        }
        ...
        public static async Task ExecuteRedo()
        {
            await new LambdaResult(c => IoC.Get<IShell>().ActiveItem.UndoRedoManager.Redo(1)).ExecuteAsync();
        }
